...
impl GraphicsPassGenerator for VertexPass {
  fn make_gpu_render_pass(
    ...
    let vert_shader = vk_manager
      .make_shader_from_spv("prism-renderer/src/vertex_pass/shaders/g_buffer.vert.spv".into())
      .map_err(|_| "Error loading triangle vert shader")?;
    let frag_shader = vk_manager
      .make_shader_from_spv("prism-renderer/src/vertex_pass/shaders/g_buffer.frag.spv".into())
      .map_err(|_| "Error loading triangle vert shader")?;
    ...
    let cam_descriptor_layout = unsafe {
      vk_manager
        .device
        .create_descriptor_set_layout(&cam_descriptor_layout_info, None)
    ...
    let tex_descriptor_layout = unsafe {
      vk_manager
        .device
        .create_descriptor_set_layout(&tex_descriptor_layout_info, None)
        .map_err(|_| "Error creating descriptor layout for vertex pipeline")?
    };

    let set_layouts = [cam_descriptor_layout, tex_descriptor_layout];
  ...
  fn create_per_frame_resources(
    ...
    for i in 0..3 {
      ...
      let attachment_view = SDImageView::new(Arc::clone(&vk_manager.device), attachment_view_info)
      ...
      let frame_buffer = SDFrameBuffer::new(Arc::clone(&vk_manager.device), frame_buffer_info)
        .map_err(|_| "Error creating frame buffer for triangle render pipeline")?;

      let descriptor_sets = descriptor_pool
        .make_sd_descriptor_sets(graphics_pass.pipeline_packs[0].descriptor_set_layouts.clone())
