...
impl SwapchainManager {
    ...
    pub unsafe fn new(
        ...
        let surface_formats = vk_manager
            .surface_driver
        ...
        let surface_capabilities = vk_manager
            .surface_driver
        ...
        let swapchain = vk_manager
            .swapchain_driver
            .create_swapchain(&swapchain_info, None)
            .ok()
            .ok_or("Error creating swapchain")?;
        if swapchain_info.old_swapchain != vk::SwapchainKHR::default(){
            vk_manager.swapchain_driver.destroy_swapchain(
                swapchain_info.old_swapchain,
                None
            );
        }
        let present_images = vk_manager
            .swapchain_driver
    ...
    pub fn blit_to_image(
        ...
        unsafe {
            ...
            vk_manager.device.cmd_blit_image(
                command_buffer,
                src_image.image,
    ...
    pub fn get_next_image(
        &self,
        vk_manager: &VKManager,
        send_semaphore: vk::Semaphore,
    ) -> Result<usize, vk::Result> {
        unsafe {
            Ok(vk_manager.swapchain_driver.acquire_next_image(
                self.swapchain,
                5000,
                send_semaphore,
                vk::Fence::null(),
    ...
    pub fn present_image(
        ...
        unsafe {
            match vk_manager.swapchain_driver.queue_present(vk_manager.g_queue, &present_info) {
                Ok(_) => {}
                Err(_) => {}
            };
        }
